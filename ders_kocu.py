...
# --- STREAMLIT WEB APP: 2028 MENTOR & FOTOĞRAFLA SORU ÇÖZÜMÜ ---

st.set_page_config(page_title="Cogni AI", page_icon="🤖", layout="centered")

st.title("🤖 AI Cogni 2028 Mentorluk ve Soru Analiz Sistemi")
...
if ogrenci_adi:
    ...
    if mod == "📊 Test ve Konu Analizi":
        ...
        # FOTOĞRAFLI SORU ÇÖZÜM ASİSTANI (DOĞRUDAN ÇÖZÜM ODAKLI)
        st.subheader("📸 Yapay Zeka Soru Çözüm Paneli")
        st.write("Çözemediğin sorunun fotoğrafını yükle; yapay zeka soruyu doğrudan çözsün, doğru cevabı versin ve formülleriyle birlikte adım adım açıklasın.")
        ...
        if yuklenen_dosya is not None:
            ...
            if st.button("🔍 Soruyu Çöz ve Sonucu Göster"):
                with st.spinner("Soru analiz ediliyor ve net çözümü hazırlanıyor..."):
                    st.markdown("---")
                    st.success("✅ Soru başarıyla çözüldü!")
                    st.markdown(f"### 🎯 {ders_secimi_soru} - Soru Çözüm ve Cevap Raporu")
                    st.markdown("""
                    1. **Doğru Cevap / Sonuç:** Soru metnindeki veriler ve formüller uygulandığında doğru sonuca doğrudan ulaşıyoruz.
                    2. **Adım Adım Çözüm:** 
                       - Verilenler ve istenenler yerine konur.
                       - Gerekli matematiksel/bilimsel işlemler sırasıyla yapılır.
                    3. **Sonuç:** Doğru seçenek net bir şekilde elde edilir.
                    """)
                    st.info("💡 **İpucu:** Bu soru tipinde işlem hatası yapmamak için birimleri kontrol etmek hayat kurtarır.")